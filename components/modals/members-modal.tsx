...
const { workspace } = data as { workspace: WorkspaceWithMembersWithProfiles };
...
const onKick = async ( memberId: string ) => {
  try {
    ...
    router.refresh();
    onOpen("members", { workspace: response.data });
  } catch (error) {
    console.log(error);
...
const onRoleChange = async (memberId: string, role: MemberRole) => {
   try {
     ...
     toast.success("Role change successfull!")
     router.refresh();
     onOpen("members", { workspace: response.data })
              ...
              {workspace?.members?.map((member) => (
                <div key={member.id} className="flex items-center gap-x-2 mb-6">
                  ...
                  <div className="flex flex-col gap-y-1">
                    <div className="text-xs fontsemibold flex items-center gap-x-1">
                      {member.profile.name}
                  ...
                  {workspace.profileId !== member.profileId && loadingId !== member.id && (
