...
const { channel, workspace } = data;
 ...
 const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      form.reset();
      toast.success(`${channel?.name} edited successfully!`)
      router.refresh();
      onClose();
    } catch (error) {
      console.log(error);
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            >
                             <FormControl>
                              <SelectTrigger
                               className="bg-zinc-300/50 b-0 focus:ring-0 text-black ring-offset-0 focus:ring-offset-0 capitalize outline-none"
                              >
